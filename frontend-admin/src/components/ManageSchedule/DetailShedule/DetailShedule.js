...
class DetailShedule extends Component {
  ...
  onSubmit = (e) => {
    if (window.confirm("Do you really want to update?")) {
      ...
      this.state.Film.map( async (item) => {
        const film = item;
        await axios.put('http://conallserver.ddns.net:8000/schedule/updateSchedule', film)
        .then((res) => {
          if (!res.data.error) {
            count++;
          } else {
  ...
  handleOnclickDelete = (film) => {
    if (window.confirm("Do you really want to delete?")) {
      var film = film;
      axios.put("http://conallserver.ddns.net:8000/schedule/deleteSchedule", film)
        .then((res) => {
          if (!res.data.error) {
            console.log(res.data)
            axios.put("http://conallserver.ddns.net:8000/ghe/updateStatus", film)
            .then((res1) => {
